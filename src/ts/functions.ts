...
export function addTodo(todoText: string, todos: Todo[]): IAddResponse {
  ...
  if (todoText.length > 2) {
    ...
    return { success: true, error: "Du måste ange minst tre bokstäver" }; //IAddRespons interfacets egenskaper används här: return ändrar success till true. // om false/error = felmedd...
  } else {
